    ...
    return <nav className="bg-white shadow-md py-4 border-b border-gray-200">
        <div className="container mx-auto flex justify-between items-center px-6 lg:px-8">
            <Link href="/" className="flex items-center space-x-4">
                <Image src={"/logo-airplane.png"} alt="logo" width={50} height={50} />
                <span className="text-2xl font-bold text-gray-800">Quem viaja mais</span>
            </Link>

            <div className="flex items-center space-x-4">
                <Link href={"/trips"} className="text-slate-900 hover:text-sky-600">Viagens</Link>
                <Link href={"/globe"} className="text-slate-900 hover:text-sky-600">Mundo</Link>

                <button className="flex items-center bg-gray-800 hover:bg-gray-900 text-white p-2 rouded-sm cursor-pointer"
                onClick={login}>
                    Sign In
                         <svg
